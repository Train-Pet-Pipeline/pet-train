...
#### 扩展数据点：1357 样本 (sft_v3b)

在标注扩展到 2026 条后（approved 1800 + needs_review 226），降采样 sitting_idle 至 800 条，得到 1357 训练样本。使用长 prompt + 5 epochs 训练 (sft_v3b)：

| 数据量 | Samples | Action Acc | Compliance | 备注 |
|--------|---------|-----------|------------|------|
| 25% | 241 | 28.6% | 100% | 消融实验 |
| 50% | 483 | 42.9% | 100% | 消融实验 |
| 75% | 724 | 61.9% | 100% | 消融实验 |
| 100% (v2) | 966 | 61.9% | 100% | 3 epochs |
| 140% (v3b) | 1357 | 61.9% | 90.5% | 5 epochs，降采样 sitting_idle |

**发现**: 数据量从 966→1357 (+40%) 后 action accuracy 未提升（均 61.9%），说明当前瓶颈已从"数据总量不足"转移到"数据分布不均"。新增的 391 条大部分是 sitting_idle/other，对 eating (16条)、drinking (2条)、leaving_bowl (1条) 等稀缺类别帮助有限。
...
训练 v3 时发现 **训练/推理 prompt 不一致会严重影响模型输出质量**。pet_schema 有两种 prompt 格式：

- **长 prompt** (~10,000 字): 由 `pet_schema.render_prompt("1.0")` 生成，user message 包含完整 Schema 字段定义、枚举值、约束
- **短 prompt** (~25 字): `请根据图片分析宠物的行为状态，输出结构化JSON。`

#### 实验矩阵

| 训练 prompt | 推理 prompt | Compliance | Action Acc | 备注 |
|------------|-----------|-----------|------------|------|
| 长 (v2) | 长 | **100%** | **61.9%** | 最佳组合 |
| 短 (v3) | 长 | 100% | 52.4% | 推理长 prompt 引导了格式，但 action 下降 |
| 短 (v3) | 短 | 85.7% | 57.1% | 缺 schema_version / other 等字段 |
| 长 (v3b) | 长 | 90.5% | **61.9%** | 追平 v2，2 个边界 case 不合规 |

#### 发现

1. **长 prompt 训练 + 长 prompt 推理效果最好**。模型在推理时能"看到"完整 Schema 定义（字段名、枚举值），相当于开卷考试。
2. **短 prompt 训练会损害语义理解**。即使推理时用长 prompt 引导格式，action accuracy 仍下降 ~10pp。
3. **推理 prompt 必须和训练 prompt 对齐**。不匹配时会丢失 `schema_version`、`other` 等字段，compliance 降至 0%。
4. **长 prompt 的端侧成本可接受**。多 ~10K 字符的输入在 RK3576 上仅增加 ~0.5s 延迟，但显著提升输出质量。

**结论**: 统一使用 `pet_schema.render_prompt()` 作为训练和推理的唯一 prompt 来源。eval 代码已增加 `prompt_source` 配置项。

### 5.9 教师 vs 学生输出对比

以下展示 4 个代表性样本上教师模型（doubao-seed-1.6-vision）、v2 学生（966 样本）和 v3b 学生（1357 样本, 长 prompt）的 narrative 对比。

#### Case 1: sitting_idle (三方一致)

| 字段 | 教师 (doubao) | 学生 v2 (966 样本) | 学生 v3b (1357 样本) |
|------|-------------|-------------------|---------------------|
| action.primary | **sitting_idle** | **sitting_idle** ✓ | **sitting_idle** ✓ |
| narrative | 一只纯黑色带绿眼睛的猫蜷缩静卧，注视镜头方向，画面带颗粒噪点，未可见食碗水碗 | 一只黑色猫咪趴在白色表面，面向左侧，眼睛带有绿色虹彩，处于静止状态，画面模糊。 | 一只黑色猫咪，头部朝向画面右侧，带绿色眼睛，处于静止状态，画面模糊，未见食碗水碗。 |

**分析**: action 三方一致。narrative 上，教师最具体（"蜷缩静卧"、"颗粒噪点"），v2 和 v3b 都识别了猫和静止状态。v3b 相比 v2 多了"未见食碗水碗"这一关键否定描述，与教师一致——说明长 prompt 中完整 Schema 定义帮助模型学会描述碗的可见性。

#### Case 2: other (三方一致)

| 字段 | 教师 (doubao) | 学生 v2 (966 样本) | 学生 v3b (1357 样本) |
|------|-------------|-------------------|---------------------|
| action.primary | **other** | **other** ✓ | **other** ✓ |
| narrative | 浅橘色猫咪尾巴竖起，朝向镜头走在客厅地毯上，旁侧有一个绿色小玩具，无进食行为。 | 浅棕色猫咪站立在地毯上，抬头看向镜头，身体放松，无可见食碗水碗。 | 浅棕色猫咪站在室内地毯上，尾巴竖起，面向镜头，画面模糊，未观察到进食行为。 |

**分析**: action 一致。教师 narrative 最丰富（注意到绿色玩具）。v3b 相比 v2 多捕捉了"尾巴竖起"这一关键体态信息，与教师一致。v2 则用"身体放松"做了更抽象的描述。两个学生都没注意到绿色玩具。

#### Case 3: sniffing_only → drinking (v2 和 v3b 同误)

| 字段 | 教师 (doubao) | 学生 v2 (966 样本) | 学生 v3b (1357 样本) |
|------|-------------|-------------------|---------------------|
| action.primary | **sniffing_only** | **drinking** ✗ | **drinking** ✗ |
| narrative | 黑棕腊肠犬站在户外木质露台，低头嗅木质地面，周围摆放多个绿植盆栽，无可见食碗水碗。 | 一只黑色杜宾犬站在木质台阶上，低头饮水，画面中无可见食碗或水碗。 | 一只黑色杜宾犬站在木质露台上，低头饮水，画面模糊，未见食盆或水盆。 |

**分析**: v2 和 v3b 犯了完全相同的错误——将"低头嗅地面"误判为"低头饮水"，narrative 也都产生了事实性错误。品种识别同样有偏差（腊肠犬→杜宾犬）。增加 40% 数据量并未修正此类错误，**根因是 drinking(2 条)和 sniffing_only(44 条)的训练样本极度不平衡**。

#### Case 4: eating → sitting_idle (v2 和 v3b 同误)

| 字段 | 教师 (doubao) | 学生 v2 (966 样本) | 学生 v3b (1357 样本) |
|------|-------------|-------------------|---------------------|
| action.primary | **eating** | **sitting_idle** ✗ | **sitting_idle** ✗ |
| narrative | 黑白花色猫咪站在笼内红色食盆前方，面向镜头，画面整体较为模糊。 | 一只黑白猫站在红色食盆旁，面向镜头，处于静止状态，画面中无可见食碗。 | 黑白猫站在红色食盆旁，面向镜头，画面模糊，未观察到进食行为。 |

**分析**: 又一个 v2 和 v3b 同误的案例。教师正确识别了猫在食盆前方（暗示进食），两个学生虽然都看到了"红色食盆"，但判断为 sitting_idle。v2 说"无可见食碗"（矛盾：前面提到食盆但又说无食碗），v3b 说"未观察到进食行为"（更连贯但仍误判）。**eating 仅 16 条训练样本，模型未学会"站在食盆前 = 可能在吃"这一推理**。

#### 教师 vs 学生总结

| 维度 | v2 学生 | v3b 学生 | 备注 |
|------|---------|---------|------|
| 高频 action (sitting_idle, other) | ✓ 接近教师 | ✓ 接近教师 | v2/v3b 表现一致 |
| 低频 action (eating, drinking, sniffing) | ✗ 明显弱 | ✗ 明显弱 | 增加数据量未改善，需定向补充 |
| narrative 语义完整度 | 中等 | 略优于 v2 | v3b 更多否定描述（"未见食碗"）和体态细节（"尾巴竖起"） |
| narrative 事实一致性 | 有矛盾 | 更连贯 | v2 出现"红色食盆旁 + 无可见食碗"矛盾 |
| Schema 结构 | 稳定 | 稳定 | 长 prompt 训练后几乎不丢字段 |
...
2. **数据分布比数据总量更重要**。从 966→1357 样本 (+40%) 后 action accuracy 未提升（61.9%），瓶颈已从总量转移到分布不均（eating 16条, drinking 2条, leaving_bowl 1条）。

3. **训练/推理 prompt 必须对齐**。使用 `pet_schema.render_prompt()` 的长 prompt（包含完整 Schema 定义）训练和推理是最佳实践，不匹配时 compliance 可降至 0%。

4. **最优超参数组合**: lr=2e-4, rank=16, 3 epochs — 在所有消融维度中，baseline 配置表现最佳。

5. **Action 识别是核心瓶颈**: 61.9% accuracy 不足以支撑可靠的健康趋势分析，根因是训练数据中 eating/drinking/leaving_bowl 极度稀缺。

6. **merge_and_unload() 是部署的必要步骤**，未合并的 PeftModel 输出完全不可用。
...
| 行动 | 预期效果 | 工作量 |
|---|---|---|
| 补标注稀缺类 eating/drinking/leaving_bowl 至 ≥100 条/类 | Action acc → 75-85% | 2-3 天标注 |
